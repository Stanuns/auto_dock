...
#include "auto_dock/dock_driver.hpp"
                            ...
                            double& yaw_update) 
   {
    ...
    void DockDriver::scan_ir(RobotState::State& nstate, double& nvx, double& nwz, const robot_interfaces::msg::DockInfraRed::ConstSharedPtr ir, 
        ...
        else if(midback == DockStationIRState::LEFT || right == DockStationIRState::LEFT || left == DockStationIRState::LEFT){
            dock_detector_--;
        ...
        else if(midback == DockStationIRState::RIGHT || left == DockStationIRState::RIGHT || right == DockStationIRState::RIGHT){
            dock_detector_++;
            next_state = RobotState::SCAN_IR;
            next_vx = 0.0;
            next_wz = 0.2;
    ...
    void DockDriver::get_ir_return(RobotState::State& nstate, double& nvx, double& nwz, const robot_interfaces::msg::DockInfraRed::ConstSharedPtr ir,
        double& linear_update){
        RobotState::State next_state;
        double next_vx;
        double next_wz;
    ...
    void DockDriver::docking_ir(RobotState::State& nstate, double& nvx, double& nwz, const robot_interfaces::msg::DockInfraRed::ConstSharedPtr ir){
        ...
        RCLCPP_INFO(this->get_logger(), "--docking_ir --docking_count_: %d, to_docking_ir_: %d", docking_count_, to_docking_ir_);
        if(docking_count_ > 400) 
        {
            ...
            if(to_docking_ir_ > 35){
                next_state = RobotState::DOCKING_IR_RIGHT;
                next_vx = 0.0;
                next_wz = 0.0;
                // docking_count_++;
                docking_count_ = 0;
            ...
            if(to_docking_ir_ > 35){
                next_state = RobotState::DOCKING_IR_LEFT;
                next_vx = 0.0;
                next_wz = 0.0;
                // docking_count_++;
                docking_count_ = 0;
    ...
    void DockDriver::docking_ir_left(RobotState::State& nstate, double& nvx, double& nwz, double& yaw_update){
        RobotState::State next_state;
        double next_vx;
        double next_wz;

        rotated_ += yaw_update;
        if(fabs(rotated_*180/M_PI) > 10){
            next_state = RobotState::DOCKING_IR_FORWARD;
            next_vx = 0.0;
            next_wz = 0.0;
            rotated_ = 0;
            linear_ = 0;
    ...
    void DockDriver::docking_ir_right(RobotState::State& nstate, double& nvx, double& nwz, double& yaw_update){
        RobotState::State next_state;
        double next_vx;
        double next_wz;
    ...
    void DockDriver::docking_ir_forward(RobotState::State& nstate, double& nvx, double& nwz, double& linear_update){
        RobotState::State next_state;
        double next_vx;
        double next_wz;

        linear_ += linear_update;
        if(fabs(linear_) > 0.05){
            next_state = RobotState::SCAN_TO_ALIGN_IR;
            next_vx = 0.0;
            next_wz = 0.0;
            linear_ = 0;
